...
const.N <- list(neurons=1,neurondesc=2,dist=3,basenoise=4,neurontype=5,params=6,posns=7,noise=8)
const.CM <- list(connmat=1,type=2,source=3,target=4,params=5,size=6,syndist=7,noise=8,
                 learningrule=9,dopno=10,dopparams=11,dopdist=12,doptype=13,dopconn=14,curtime=15,synactfn=16)
...
#basenoise: distribution parameters c(dist,params) dist: of noise, 0 - no noise, 1 - gaussian, 2 - uniform; 
...
#op=1, 
newNeurons <- function(number=1,basenoise=c(0,0),neurontype=1,neurondesc='excitatory',posns=0,dist=1,params=c(10,1),exparams=c(),
                       noise=c(0,0,0),noStimuli=1,op=1){
  ...
  cells <- list(neurons,neurondesc,dist,basenoise,neurontype,params,posns,noise)
  names(cells) <- c('neurons','neurondesc','dist','basenoise','neurontype','params','posns','noise')
  res <- structure(cells,class = 'Neurons') #make it a class and return it
  res
}

#function that generates neurons according to the parameters specified. Better this way, as it can then be used 
#by various functions
genNeurons <- function(number,basenoise=c(0,0),dist=1,params=c(10,1),noise=list(0,0,0),noStimuli=1){
  ...
  if(dist>0 && dist<=4){#firing rate is based on a distribution
  ...
  #now add noise
  if(basenoise[1]>=1 && basenoise[1]<=2){ #add noise to the neurons
    neurons <- lapply(1:noStimuli, function(i){
      temp <- neurons[i] *(1 + switch(basenoise[1],rnorm(length(neurons[i]),0,basenoise[2]),
                                      runif(length(neurons[i]),min = basenoise[2],max = basenoise[3])))
...
newConnMatrix <- function(size=c(1,1),type=1,syndist=3,params=c(1,1.15,.16),dop=c(),
                          source='source',target='target',noise=list(0,0,0),learningRule=c(1/38,2,0.2,T)){
  ...
  if(length(dop) > 0){#this circuit does learning, so set up those params too
    ...
    connMatrix <- list(connMat,type,source,target,params,size,syndist,noise,learningRule,dopNo,dopParams,dopDist,dopType,dopConn,curTime,synactfn)
    names(connMatrix) <- c('connMat','type','source','target','params','size','syndist','noise','learningRule','dopNo','dopParams','dopDist'
                           ,'dopType','dopConn','curTime','synActFn')
    res <- structure(connMatrix, class = c('Learning','ConnMatrix')) #it is a learning connection matrix
  }
  else {#not a learning matrix
    connMatrix <- list(connMat,type,source,target,params,size,syndist,noise,learningRule)
    names(connMatrix) <- c('connMat','type','source','target','params','size','syndist','noise','learningRule')
    res <- structure(connMatrix, class= 'ConnMatrix')
  }
...
newGlomMBcircuit <- function(glomno=50,kcno=2000,danno=5,mbonno=1,noodors=4,params=c(),learningRule=c(1/38,2,0.2,T),
                             ...
                             novalences=2,tatparams=c(1,1),op=1){
  ...
  mbkc <- newNeurons(number = kcno,dist = 0,params = c(0),noStimuli = noodors)
  mbkc_net <- newNeurons(number = kcno,dist = 0,params = c(0),noStimuli = noodors)
  kcapl_conn <- newConnMatrix(c(kcno,1),syndist = 6,dop=c(),params = 1)
  apl <- newNeurons(number = 1,dist = 0,params = c(0),noStimuli = noodors,neurondesc = 'inhibitory')
  ...
  # # dans = # valencs
  dans <- lapply(1:novalences, function(i) newNeurons(number = danno,dist = 0,params = c(1)) )
  names(dans) <- valences
  # mbons = # valences, each mbons also contains the mbon firing rates given a particular odor.
  mbons <- lapply(1:novalences, function(i) newNeurons(number = mbonno,dist = 0,params = c(0),noStimuli = noodors) )
  ...
  res <- structure(glomMbCircuit,class = c('FlyMBg','Circuit')) #make it a class and return it
  res
}
...
#learningrule: the rule for updating the strength of the synapses.(alpha,beta,ec50,T:sat fn,F:linear function) additive decrease s-alpha, multiplicative inc: s*beta 
newOBPcxcircuit <- function(glomno=200,pcno=2800,noodors=4,obpcparams=c(),pcpcparams=c(),pcobparams=c(),obobparams=c(),op=1){
...
init.FlyMBg <- function(self,aplCut=5,op=1){
  mbkc <- computeFiring(getData(self,const.FM$glom),getData(self,const.FM$glommb_conn)) #get the KC firing rates
  mbkc.neurons <- getData(mbkc,const.N$neurons) #just the neurons to calculate the apl cutoff
  apl.vec <- sapply(1:getData(self,const.FM$noodors), function(i){
    #basically sets the connection strenghts for the kc <-> apl n/w so that you get aplCut percentage of KCs
    #fix the apl connection strengths, the kcapl_conn and aplkc_conn strengths
    #doesnt matter, but the strength from 
    getTopNPercentileThresh(mbkc.neurons[[i]],topn = aplCut,op=2)/sum(mbkc.neurons[[i]])
  })
  #cat('\napl vec: ',apl.vec,'\nap cut',aplCut)
  #build a connection matrix and set and assign it to Kcapl_conn
  temp <- genConnMat(size(self$apl,op=2),size(self$mbkc,op=2),type = 6,params = mean(apl.vec))
  kcapl_conn <- setData(getData(self,const.FM$kcapl_conn),val=temp,index = const.CM$connmat)
  circuit.mb <- setData(self,val=kcapl_conn,const.FM$kcapl_conn)
  #now figure out how the connection strength should be updated multiplicatively. What we want to ensure is that 
  #when KC firing is nearly 0 and thus update factor is nearly 0, there is little change in the strength, and the maxumum possible
  #change is 2 when neuron input and dop strength are together the highest. Do all valences
  #compute the firing rates of all neurons
  circuit.mb <- computeFiring(circuit.mb,op=1)
  mbkc_net <- getData(circuit.mb,const.FM$mbkc_net) #get the mbkc_net neurons
  #print(circuit.mb$mbkc$neurons[1:2])
  #print(circuit.mb$mbkc_net$neurons[1:2])
  mbkc_net.neurons <- getData(mbkc_net,const.N$neurons) 
  valences <- getData(self,const.FM$valences)
  for(i in 1:length(valences)){
    circuit.mb <- normSynWts(circuit.mb,mbkc_net.neurons,valence = valences[i],aplCut = aplCut)
  }
...
setSparsity.FlyMBg <- function(self,aplCut=5,op=1){
  ...
  apl.vec <- sapply(1:getData(self,const.FM$noodors), function(i){
    ...
    #fix the apl connection strengths, the kcapl_conn and aplkc_conn strengths
    #doesnt matter, but the strength from 
    #cat('\n',mbkc.neurons[[i]],' : ',getTopNPercentileThresh(mbkc.neurons[[i]],topn = aplCut,op=2),sum(mbkc.neurons[[i]]))
    getTopNPercentileThresh(mbkc.neurons[[i]],topn = aplCut,op=2)/sum(mbkc.neurons[[i]])
  })
  #cat('\nsetSpa',apl.vec)
  #build a connection matrix and set and assign it to Kcapl_conn
...
createFlyCircuit <- function(glomno = 5,kcno = 10,danno = 5,mbonno = 1,noodors=4,novalences=2,aplCut=5,learningRule=c(1/38,2,0.2,T),
                             ...
                             trainset=1,noiseset=1,combnop=2,op=1){
  #cat('\ncreatFly',glomno,kcno)
  circuit <- newGlomMBcircuit(noodors = noodors,glomno = glomno,kcno = kcno,danno = danno,mbonno = mbonno,novalences = novalences,
                              glommb_conn = glommb_conn,tatparams = tatparams)
...
exploreNoiseSeqRuns <- function(self,noruns,notrials,noiserange=seq(0,0.3,0.1),noiseparams=list(1,c(0,0.1),1),
                            noiseset=1,outcomp=1,op=1,resop=4){
  res.ls <- lapply(1:noruns,function(x){
    res <- exploreNoiseSeq(self = self,notrials = notrials,noiserange = noiserange,noiseparams = noiseparams,
                           noiseset = noiseset,outcomp = outcomp,op = op,resop = resop)
  })  
...
#op: specifies the compoenents to which noise is to be added
#targetpars: the target parameters against which you should be comparing results, the default are the fly params,
#chooseop:
#cirop: 
...
exploreNoiseParams<-function(noruns,notrials,noodors=6,noiserange=seq(0,0.3,0.1),noiseparams=list(1,c(0,0.1),1),
                             aplrange=seq(5,15,5),glomrange=c(50),kcrange=c(100),noiseset=2,outcomp=1,op=1,resop=1,
                             chooesop=2,retop=1,
                             targetpars=list(c(0.72,0.07),c(5.26,0.7),c(7.23,0.84),c(6.1,1.9),c(29,3.16)) ){
  #create the circuit
...
#parno: the param no,i.e., the row to look at
...
#op: 1 - return value, 2 - noise percent, 3 - c(value,noise percent)
cirGetParamsTarget <- function(dat.ls,parno=1,targetpars,chooseop=1,op=3){
  ...
  close <- sapply(1:ncol(allpars[[1]]),function(i){
    #cat('\t',c(allpars[[1]][parno,i],allpars[[2]][parno,i]))
    overlap <- intervalOverlap(targetpars,c(allpars[[1]][parno,i],allpars[[2]][parno,i]),inputop = 2)
  })
  if(sum(close)==0) return(switch(op,0,0,c(0,0)) )
  over.amt <- switch(chooseop,min(close[which(close>0)]),max(close))
  opt <- which(close==over.amt)
  cat('\noverlap',close,'\nopt',over.amt,';',opt)
  #res <- names(allpars[[1]])[which(close==opt)]
  res <- names(allpars[[1]])[opt]
  cat('\n',names(allpars[[1]])[opt])
  #res.nice <- cirConvNoiseStr(res)
  #switch(op,allpars[[1]][parno,res],res.nice,c(allpars[[1]][parno,res],res.nice))
  res
}
...
cirGetAllParamsTarget <- function(dat.ls,targetpars,chooseop=1,def.width=0.15,op=2){
  ...
  res.ls <- lapply(1:ncol(allpars[[1]]),function(i){#iterate through parameter combination
    parscore <- sapply(1:nrow(allpars[[1]]),function(j){#through each of the constraints
      #score <- cirGetParamsTarget(dat.ls = dat.ls,parno = j,targetpars = targetpars[[j]],chooseop = choosop)
      #cat('\ncirGetAllParamsTarget',targetpars[[j]],',',c(allpars[[1]][j,i],allpars[[2]][j,i]) )
...
#result: 
#inputop: input, e.g., int1 is of the form c(x1,x2) or 2 = c(mn,sem) so it is c(mn-sem,mn+sem)
...
intervalOverlap<-function(interval1,interval2,inputop=1,op=1){
  inter1 <- cleanNAVec(interval1);inter2 <- cleanNAVec(interval2)
  int1 <- convertMeanWidthInterval(inter1[1],inter1[2],op = inputop)
  int1m <- mean(int1) #mean needed later for normalization
  int2 <- convertMeanWidthInterval(inter2[1],inter2[2],op = inputop)
  #cat('\ninterover',inter1,';',inter2)
  #cat('\nintover',int1,';',int2)
  ...
  #cat('\nres',int1m,';',abs(mean(int2) - mean(int1))/int1m)
  if(op==2) return(abs(mean(int2) - mean(int1))/int1m )   
  # the overlap is either the upper value of int1 or int2 - lower value of int2
  if(int1[2] < int2[2]) res <- (int1[2]-int2[1])/int1m
  else res <- (int2[2]-int2[1])/int1m 
  # if(int1[2] < int2[2]) res <- (int1[2]-int2[1])/(int2[2]-int1[1])
  # else res <- (int2[2]-int2[1])/(int1[2]-int1[1]) 
  #res <- (min(int1[2],int2[2])-max(int1[1],int2[1]))/(max(c(int1,int2))-min(c(int1,int2)))
  res
}
...
#liam fix it so that we are varying noise for multiple params at the same time
...
exploreNoiseSeq <- function(self,notrials,noiserange=list(seq(0,0.3,0.1)),noiseparams=list(1,c(0,0.1),1),
  ...
  if(op>=10 && op<=11) {#determine #components and generate the parameter combination
    nocomp <- switch(op-9,2,3) #pn, pn-kc; pn, pn-kc, apl-kc 
    #basically if this is a single list, for a mult-component thing, we need a multi-list
    #cat('\nf',isDataType(noiserange)==1,isDataType(noiserange)==4,length(noiserange)>1)
  ...
  #if(op>=10){#different combinations of noise different parameters
  res.ls <- lapply(1:nrow(noisevals),function(i){
...
#todo: have to make it so that we can add different amounts of noise to the various multi-component tests like 10 & 11  
...
exploreNoise.FlyMBg <-function(self,notrials,noiseparams=list(1,c(0,0.1),1),noiseset=1,outcomp=1,op=1,resop=1){
  ...
  #circuit <- addNoise(circuit,op=op)
  odornos <- chooseStimuli(circuit,stimset = noiseset)
  #cat('\nexploreNoise.FlyMBg',notrials,'trials',1:(notrials-1))
  if(notrials>1) {
  ...
  switch(resop,processNoiseResults(res,stim = odornos,comp=3),processNoiseResults(res,stim = odornos,comp=2),
...
#op: components to which we want to add noise; 1, glom/pns, 2 - PN-MB, 3 - KC-APL, 4 - APL-KC
#5 - kc-mbon connection matrix, 6 - mbons, 7 dans, 10 - PN & PN-MB, 11 - PN, PN-MB, APL-KC, 
#12 - DAN-KC-MBON connections, 100 - noise to everything, 0 no noise
setNoiseParams <- function(self,noiseparams=list(1,c(0,0.1),1),op=1){
  ...
  if(op>=1 && op<=4){#only one component or structure for noise addition
    ...
    circuit <- setNoiseComp(compid = compid,circuit = circuit,noiseparams = noiseparams)
  }
  ...
  if(op==100){#set noise to all structures
    ...
    for(i in seq_wrap(1,length(compids))){
      circuit <- setNoiseComp(compid = compids[i],circuit = circuit,noiseparams = noiseparams)
    }
    #now, set the valence related bits
    ...
    for(j in seq_wrap(1,length(compids))){
      circuit <- setNoiseComp(compid = compids[j],circuit = circuit,noiseparams = noiseparams)
    }    
  }
  circuit
}
...
setNoiseComp <- function(compid,circuit,noiseparams,op=1){
  ...
  #cat('\nsetnoisetype',comptype)
  if(comptype==1) comp <- setData(comp,val=noiseparams,const.N$noise) #set its noise parameter
...
computeFiring.Neurons <- function(self,connMat,dest=c(),stimno=1,op=1){
...
#noiseop: components to which we want to add noise; 0 - no noise, 1, glom/pns, 2 - PN-MB, 3 - KC-APL, 4 - APL-KC
#5- mb-mbon connection matrix, 6 - mbons, 7 - dans, 10 - PN & PN-MB, 11 - PN, PN-MB, APL-KC, 
#12 - DAN-KC-MBON connections, 13 - noise to everything, 0 no noise, 
#100 - add noise according to noise params to every structure
...
computeFiring.FlyMBg <- function(self,odor=1,noiseop=0,op=2){
  ...
  #components of the circuit, and then do compute firing. It wont change self's firing rates, but will change, newcircuit's
  #firing rates
  ...
  #glom <- setData(glom,val=list(glom$neurons[[odor]]),index=const.N$neurons)
  ...
  #mbkc_net <- assignObject(getData(self,const.FM$mbkc_net),mbkc_net,index=const.N$neurons) #set the new firing rates for mbkc_net
  ...
  for(i in 1:length(kcmbon_conn.lst) ){
    mbon.lst[[i]] <- computeFiring(mbkc_net,kcmbon_conn.lst[[i]],mbon.lst[[i]],stimno=odor,op=op)
  }
...
computeFiringLayer.FlyMBg <- function(self,layer=1,odor=1,op=2){
  ...
  if(layer==3 || layer==4){#special processing for calculating mbkc_net or mbons
    if(layer==3){#mbkc_net apl feedback
      target <- twoSetOperations(components[[5]],computeFiring(components[[1]],components[[2]]))
...
twoSetOperations.Neurons <- function(self,second,op=1){
  origin <- self
  ...
  origin$neurons <- neurons
  origin #return the result
}
...
getData.ConnMatrix <- function(self,datatype=1,op=1){
  ...
  switch(op,switch(datatype,
                   self$connMat,self$type,self$source,self$target,self$params,self$size,self$syndist,self$noise,self$learningRule,
                   self$dopNo,self$dopParams,self$dopDist,self$dopType,self$dopConn,self$curTime,self$synActFn),
         switch(datatype,
                'connmat','type','source','target','params','size','syndist','noise','learningRule',
...
getData.Neurons <- function(self,datatype=1,stimno=c(),op=1){
  switch(op,switch(datatype,
                   self$neurons,self$neurondesc,self$dist,self$basenoise,self$neurontype,self$params,self$noise), #actual data types
         switch(datatype,'neurons','neurondesc','dist','basenoise','neurontype','params','noise'),#datatype descriptions
         self$neurons[[stimno]]
  )
}
...
addNoise.Neurons <-function(self,noiseparams=c(),stim=c(),op=1){
  ...
  res.neurons <- lapply(stimnos,function(i){
    #cat('\nnoise call',stim.neurons[[i]],'dist',noise[[1]],'noise',str(noise[[2]][2]))
    ...
    #cat('\naddnoise neurons',odor.neurons[[i]],'\nnoise ',noisefactor,'\t',neuron)
    neuron
  })
...
addNoise.FlyMBg <-function(self,odors=c(),noiseset=0,op=1){
  ...
  if(op>=1 && op<=4){#only one component or structure for noise addition
    compid <- switch(op,const.FM$glom,const.FM$glommb_conn,const.FM$kcapl_conn,const.FM$aplkc_conn,
                             const.FM$dans,const.FM$kcmbon_conn)
    circuit <- addCompNoise(circuit,compid = compid,stimnos = odornos)
  }
  ...
  if(op==100){#add noise to all structures
    ...
    #got through each component, then their valences, and set noise
    for(j in seq_wrap(1,length(compids))){
      circuit <- addCompNoise(circuit,compid = compids[j],stimnos = odornos)
    }    
  }
  circuit
}
...
addCompNoise<-function(self,compid,stimnos,op=1){
  ...
  if(comptype==1) circuit <- setData(circuit,val=addNoise(comp,stim=stimnos),compid)
...
#genFlyPNMBconn(rows = rows,cols = cols,glomnopars = params[[1]],glompars = params[[2]],synpars = params[[3]])
genConnMat <- function(rows,cols,params=c(1,1.15,.16),type=3,noise=list(0,0,0),op=1){
  ...
  if(type==7){#each source neurons makes "n_i" connections, where n is derived from a Poisson distribution with mean "n" 
    mat <- generateIndSynMatrices(target = rows,source = cols,params)
  }
  ...
  mat*(1 + genNoiseMat(rows = rows,cols = cols,noisedist = noise[[1]],noise = noise[[2]][2],noisetype = noise[[3]]))
...
generateIndSynMatrices <-function(source,target,params=list(4,c(1,1.15,0.16)),op=1){
  targets <- 1:target
  mats.ls <- lapply(targets, function(i){
    ...
    nosyn <- rpois(source,params[[1]])
    maxsyn <- max(nosyn)
    #cat('\nsynapse:',nosyn)
    #each source neuron makes nosyn synapses with the target neuron
    #so matrix with nosyn rows and source number of cols
    mat <- matrix(genSynapses(maxsyn*source,distop=op,params = params[[2]]),ncol = source)
...
updateSynMat <- function(mat,nosyn,op=1){
  ...
  norows <- nrow(mat)
  noempty <- rep(norows,length(nosyn)) - nosyn
...
#distop: 1 - gamma distibuteion of strngth, 2 - unform distibution of strength
